...
void predict_three_para(short **image_construct, short **resi, short **predicted, int height, int width, int mode)
{
	...
	case 0: case 1: case 2: case 3: case 4: case 5: case 6: case 7:
		estimate_LeftTop_three(image_construct, para, width, height, mode);
		break;
	case 8:
		estimate_LeftDown_three(image_construct, para, width, height, mode);
		break;
	default:
		break;
	}
	...
	{
		case 0: case 1: case 2: case 3: case 4: case 5: case 6: case 7:
			predict_LeftTop_three(image_construct, resi, predicted, para, height, width, mode);
			break;
		case 8:
			predict_LeftDown_three(image_construct, resi, predicted, para, height, width, mode);
			break;
		default:
			break;
	}
...
void AccumulateParaAB_three(double **paraA, double **paraB, short block_image[padded_imageblock_height][padded_imageblock_height], int x, int y, int mode)
{   //需要调整多个块累加的程序。。。。??
		...
		for (int i_col = 0; i_col < COLS; ++i_col)
		{
			...
			for(int i = bias_top_row; i < BLOCKHEIGHT + bias_top_row; ++i)
				for(int j = bias_left_col; j < BLOCKWIDTH + bias_left_col; ++j)
					AccumulateParaAB_three(paraA, paraB, block_image[i_row][i_col], j, i, mode);
	...
	for (int i_row = 0; i_row < ROWS; ++i_row)
	{
		for (int i_col = 0; i_col < COLS; ++i_col)
		{
			...
			for(int i = bias_top_row; i < BLOCKHEIGHT + bias_top_row; ++i)
			{
				for(int j = bias_left_col; j < BLOCKWIDTH + bias_left_col; ++j)
				{
					short North, NW, W;	// u[y][x] equals to u(x, y)
					switch (mode)
					{
					case 0: 
						North = block_image[i_row][i_col][i - 1][j + 1];
						NW = block_image[i_row][i_col][i - 1][j];
						W  = block_image[i_row][i_col][i - 1][j - 1];
						break;
					case 1: 
						North = block_image[i_row][i_col][i - 1][j - 1];
						NW = block_image[i_row][i_col][i][j - 1];
						W  = block_image[i_row][i_col][i + 1][j - 1];
						break;
					case 2:
						North = block_image[i_row][i_col][i - 1][j];
						NW =    block_image[i_row][i_col][i - 1][j - 1];
						W  =    block_image[i_row][i_col][i][j - 1];
						break;
					case 3:
						W =     block_image[i_row][i_col][i - 1][j];
						North = block_image[i_row][i_col][i - 1][j + 2];
						NW =    block_image[i_row][i_col][i - 1][j + 1];
						break;
					case 4:
						//4模式可以用下面公式计算
						North =  block_image[i_row][i_col][i - 1][j];
						NW    =  block_image[i_row][i_col][i - 1][j - 1];
						W     =  block_image[i_row][i_col][i - 1][j - 2];
						break;
					case 5:
						//5模式可以用下面公式计算
						North = block_image[i_row][i_col][i - 2][j];
						NW    = block_image[i_row][i_col][i - 2][j - 1];
						W     = block_image[i_row][i_col][i - 2][j - 2];
					...
					case 6:
						//6模式可以用下面公式计算
						North = block_image[i_row][i_col][i - 1][j - 1];
						NW    = block_image[i_row][i_col][i - 1][j - 2];
						W     = block_image[i_row][i_col][i - 1][j - 3];
						break;
					case 7:
						North = block_image[i_row][i_col][i - 2][j + 2];
						NW    = block_image[i_row][i_col][i - 2][j + 1];
						W     = block_image[i_row][i_col][i - 2][j];
						break;
					default:
						break;
					}

					block_pre[i - bias_top_row][j - bias_left_col] = (short)(para[0][0] * W + para[1][0]* NW + para[2][0] * North);
	...
	for (int i_col = 0; i_col < COLS; ++i_col)    //遍历顺序很重要！！！！！从上到下，从第一列到最后一列
	{
		for (int i_row = 0; i_row < ROWS; ++i_row)
		{
			...
			for(int i = bias_top_row; i < BLOCKHEIGHT + bias_top_row; ++i)
			{
				for(int j = bias_left_col; j < BLOCKWIDTH + bias_left_col; ++j)
				{
					North = (block_image[i_row][i_col][i][j - 1] + block_image[i_row][i_col][i - 1][j - 1]) / 2;
					NW = block_image[i_row][i_col][i - 1][j];
					W = (block_image[i_row][i_col][i + 1][j - 1] + block_image[i_row][i_col][i][j - 1]) / 2;
