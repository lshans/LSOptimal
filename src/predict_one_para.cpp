...
void predict_one_para(short **image_construct, short **resi, short **predicted, int height, int width, int mode)
{
	...
	case 0: case 1: case 2: case 3: case 4: case 5: case 6: case 7:
		estimate_LeftTop_one(image_construct, para, width, height, mode);
		break;
	case 8:
		estimate_LeftDown_one(image_construct, para, width, height, mode);
		break;
	default:
		break;
	}
	...
	{
		case 0: case 1: case 2: case 3: case 4: case 5: case 6: case 7:
			predict_LeftTop_one(image_construct, resi, predicted, para, height, width, mode);
			break;
		case 8:
			predict_LeftDown_one(image_construct, resi, predicted, para, height, width, mode);
			break;
		default:
			break;
	}
...
void AccumulateParaAB_one(double **paraA, double **paraB, short block_image[padded_imageblock_height][padded_imageblock_height], int i, int j, int mode)
{   //需要调整多个块累加的程序。。。。??
		...
		for (int i_col = 0; i_col < COLS; ++i_col)
		{
			...
			for(int i = bias_top_row; i < BLOCKHEIGHT + bias_top_row; ++i)
				for(int j = bias_left_col; j < BLOCKWIDTH + bias_left_col; ++j)
					AccumulateParaAB_one(paraA, paraB, block_image[i_row][i_col], i, j, mode);
		}
	}
	cout << "para end " << endl;
...
long long predict_LeftTop_one(short **image_construct, short **resi, short **predicted, double **para, int height, int width, int mode)
{
	...
	for (int i_row = 0; i_row < ROWS; ++i_row)
	{
		for (int i_col = 0; i_col < COLS; ++i_col)
		{
			...
			//		break;
			//}
			int block_energy = 0;                   //每一个小块的残差能量值
			...
			for(int i = bias_top_row; i < BLOCKHEIGHT + bias_top_row; ++i)
			{
				for(int j = bias_left_col; j < BLOCKWIDTH + bias_left_col; ++j)
				{
					...
					{
					case 0:
						u[1] = (block_image[i_row][i_col][i - 1][j] + block_image[i_row][i_col][i - 2][j]) / 2;
						break;
					case 1:
						u[1] = (block_image[i_row][i_col][i][j - 1] + block_image[i_row][i_col][i][j - 2]) / 2;
						break;
					case 2:
						u[1] = (block_image[i_row][i_col][i - 1][j - 1] + block_image[i_row][i_col][i - 1][j] + block_image[i_row][i_col][i][j - 1]) / 3;
						break;
					case 3:
						u[1] = (block_image[i_row][i_col][i - 1][j] + 2 * block_image[i_row][i_col][i - 1][j + 1] + block_image[i_row][i_col][i - 1][j + 2] + 2) / 4;
						break;
					case 4:
						u[1] = (block_image[i_row][i_col][i - 1][j - 2] + 2 * block_image[i_row][i_col][i - 1][j - 1] + 2 * block_image[i_row][i_col][i - 1][j] + 2) / 4;
						break;	
					case 5:
						u[1] = (block_image[i_row][i_col][i - 1][j] + block_image[i_row][i_col][i - 1][j - 1] + 1) / 2;
						break;	
					case 6:
						u[1] = (block_image[i_row][i_col][i][j - 1] + block_image[i_row][i_col][i - 1][j - 1] + 1) / 2;
						break;	
					case 7:
						u[1] = (block_image[i_row][i_col][i - 1][j] + block_image[i_row][i_col][i - 1][j + 1] + 1) / 2;   
						break;		
					...
					block_pre[i - bias_top_row][j - bias_left_col] = (short)(para[0][0] * u[1]);
